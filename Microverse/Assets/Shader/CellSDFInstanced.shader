    ...
    SubShader{
        ...
        Pass{
            ...
            UNITY_INSTANCING_BUFFER_START(Props)
                UNITY_DEFINE_INSTANCED_PROP(float4, _Color) // 기존 per-instance 색
                UNITY_DEFINE_INSTANCED_PROP(float,  _Phase) // ★ 추가: 0..1 위상
            ...
            fixed4 frag (v2f i) : SV_Target{
                ...
                float  phase   = UNITY_ACCESS_INSTANCED_PROP(Props, _Phase); // ★ 인스턴스별 위상
                ...
                // 막 요철 + 맥동 + 흔들림 (위상 적용)
                float pulse    = 1.0 + _PulseAmp   * sin(_PulseFreq * t + phase * 6.2831);
                ...
                cyt.rgb = lerp(cyt.rgb, mainCol.rgb, 0.35);
