...
namespace Microverse.Scripts.Simulation.Runtime.Systems
{
    ...
    {
        const byte SPEC_CELL = 0;     // 일반 세포
        const byte SPEC_WBC = 1;     // 백혈구
        const byte SPEC_BACT = 2;   // 박테리아(플레이어 종)
        ...
        void Draw()
        {
            ...
            for (int i = 0; i < agentCount; i++)
            {
                ...
                {
                    c = new Color(1f, 0.4f, 0.4f, 1f); // 라치된 WBC 강조
                }
                else if (B.species[i] == SPEC_BACT)
                {
                    c = new Color(0.2f, 0.9f, 0.8f, 1f); //  박테리아(시안/민트톤)
                }
                else
                {
                    c = colorBySpecies.Evaluate(B.species[i]); // 기존 일반/백혈구 그라데이션
                }
